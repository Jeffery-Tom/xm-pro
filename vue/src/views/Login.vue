...
<!--        所以 data.form 本质上就是一个被 Vue 监听的 JS 对象，用来管理表单数据。-->
        <el-form ref="formRef" :rules="data.rules" :model="data.form" style="width: 350px">
          <div style="margin-bottom: 30px; font-size: 24px;text-align: center; color: #02457e; font-weight: bold">欢迎登录后台管理系统</div>
...
<!--         text-align让div往右显示                                        text-decoration去掉a标签超链接的下划线-->
          <div style="text-align: right">还没有账号? 请 <a style="color: #247aea;text-decoration: none " href="/register" >注册</a></div>
        </el-form>
      </div>
  ...
  formRef.value.validate((valid) => {
    ...
    if(valid) {
      request.post('/login',data.form).then(res =>{
        if(res.code === '200'){
          //存储后台返回的用户数据信息
          localStorage.setItem('xm-pro-user',JSON.stringify(res.data)) //把JSON对象转换成字符串存储
          ElMessage.success('登陆成功')
          setTimeout(() => {
            location.href = '/manager/home' //登陆成功跳转主页
          },500)
